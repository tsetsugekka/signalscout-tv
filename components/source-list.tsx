...
export function SourceList({evaluation,channel,checks,hideFailed,activeSource,onSelect,onRecheck}:{evaluation:EvaluationContext;channel:Channel;checks:Record<string,SourceCheck>;hideFailed:boolean;activeSource?:string;onSelect:(id:string)=>void;onRecheck:()=>void}){
 ...
 const hint=result.current.status==='possible'?`同主机有${deviceLabel(result.current.hintDevice!)}成功记录，本线路仍待验证`:undefined;
 return <div key={s.id} className={`source-row source-${displayStatus} ${activeSource===s.id?"source-active":""}`}><div className="source-choice"><RadioGroupItem value={s.id} id={`source-${i}`} disabled={check.status==="unsupported"}/><label htmlFor={`source-${i}`}><strong>线路 {i+1}{activeSource===s.id&&<small>当前</small>}<span className="source-mode">{s.url.startsWith("http:")?"HTTPS 转发":"直连"}</span></strong></label><span className="source-result" title={hint}>{busy?<LoaderCircle className="spin"/>:isSuccess(result.display)?<CheckCircle2/>:["unstable","unsupported"].includes(displayStatus)?<XCircle/>:<Clock3/>}{label}</span></div><div className="source-device-status">{(["pc","mobile"] as const).map(kind=>{const evidence=result.devices[kind],community=sharedEvidence(deviceHealth[s.id]?.[kind],evaluation.now);const conflict=evidence.origin==='local'&&community.status!=='unknown'&&isSuccess(evidence)!==isSuccess(community);return <span key={kind} className={`device-${evidence.status}`}>{deviceLabel(kind)}：{evidenceLabel(evidence)}{conflict&&<small className={`device-${community.status}`}> · {evidenceLabel(community)}</small>}</span>;})}</div><div className="source-address"><code>{s.url}</code><div className="source-url-actions"><button onClick={()=>void copy(s.id,s.url)}>{copied===s.id?"已复制":"复制地址"}</button><a href={s.url} target="_blank" rel="noreferrer">打开原始地址 ↗</a>{check.connectMs!==undefined&&<span>{Math.round(check.connectMs)} ms</span>}</div></div>{check.reason&&<p className="source-meta">{check.reason}</p>}</div>;
 })}</RadioGroup>{pages>1&&<nav className="source-pagination" aria-label="播放线路分页"><button className="quiet-button" disabled={currentPage===0} onClick={()=>setPage(currentPage-1)}>上一页</button><span aria-live="polite">{currentPage+1} / {pages}</span><button className="quiet-button" disabled={currentPage===pages-1} onClick={()=>setPage(currentPage+1)}>下一页</button>{activePage>=0&&activePage!==currentPage&&<button className="source-locate" onClick={()=>setPage(activePage)}>定位当前线路</button>}</nav>}{hidden>0&&<p className="source-help">已隐藏 {hidden} 条失败或不稳定线路；关闭“不显示失效源”可查看。</p>}{copyError&&<p role="status" className="source-help">{copyError}</p>}<p className="source-help source-explainer" title="跨域检查失败不代表源离线。离线或后台时暂停检测。">选择线路后在当前设备验证播放。</p></section>;
}
