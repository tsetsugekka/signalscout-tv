...
export function useTV(){
 ...
 const updateCheck=(id:string,result:SourceCheck)=>{if(!(id in checksRef.current))return;checksRef.current={...checksRef.current,[id]:result};setSourceChecks(checksRef.current);};
 ...
 useEffect(()=>{
  ...
  const refresh=async(force=false)=>{
   ...
   }catch{if(alive){setSyncMessage("");setNotice(catalogRef.current.channels.length?"目录暂时无法更新，已保留现有频道。":"尚未取得频道目录，请点击更新目录重试。");}}
   finally{refreshing=false;if(alive)setCatalogLoading(false);}
  };refreshRef.current=refresh;
  void (async()=>{
   try{const [saved,cached]=await Promise.all([readLocal<LocalState>("preferences"),readLocal<Catalog>("catalog")]);if(!alive)return;if(saved)data.current={...defaults(),...saved,favorites:[...new Set(saved.favorites.map(normalizeName))],recent:[...new Set(saved.recent.map(normalizeName))],lastChannel:normalizeName(saved.lastChannel),lastPlayedSources:Object.fromEntries(Object.entries(saved.lastPlayedSources||{}).map(([id,url])=>[normalizeName(id),url]))};if((data.current.relayVersion||0)<3){data.current.unavailable={};for(const [url,health] of Object.entries(data.current.health))if(url.startsWith("http:")&&(health.failedAt||0)>(health.okAt||0)){if(health.verifiedLive&&health.okAt)data.current.health[url]={okAt:health.okAt,verifiedLive:true,failures:0,until:0};else delete data.current.health[url];}data.current.relayVersion=3;}if(cached?.channels?.length)acceptCatalog(cached);}catch{storageError();}
   if(!alive||!video.current)return;setPrefs({...data.current});
   engine.current=new PlayerEngine(video.current,data.current,s=>{if(alive)setPlayback(s);},persist,stopProbe,(id,result)=>{if(alive)updateCheck(id,result);});engine.current.auto=data.current.autoSwitch;
 ...
 const hasFailed=(c:Channel)=>{const candidates=browserSources(c);return candidates.length>0&&candidates.every(s=>c.id===selectedId&&sourceChecks[s.id]?sourceChecks[s.id].status==="failed":(prefs.health[s.id]?.until||0)>Date.now()&&(prefs.health[s.id]?.failedAt||0)>(prefs.health[s.id]?.okAt||0));};
 ...
 return {catalog,prefs,selected,playback,probingId,sourceChecks,selectSource,recheckSources,setHideFailed,hasFailed,notice,catalogLoading,syncMessage,ready,category,setCategory,video,selectChannel,toggleFavorite,setAuto,isUnavailable,lastSuccess,backupCount,retry:recheckSources,resume:()=>engine.current?.resume(),unmute:()=>{if(video.current)video.current.muted=false;},refresh:()=>refreshRef.current?.(true)};
}
