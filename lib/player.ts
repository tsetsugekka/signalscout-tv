...
export function orderedSources(channel:Channel,state:LocalState,attempted=new Set<string>()){
 const now=Date.now(),preferred=state.lastPlayedSources[channel.id];return browserSources(channel).filter(s=>!attempted.has(s.id)&&(s.id===preferred||!(state.health[s.id]?.until>now))).sort((a,b)=>{
  ...
  const score=(s:Source)=>{const t=state.health[s.id]?.verifiedLive?state.health[s.id].okAt||0:0;return now-t<24*3600_000?t:0;};return score(b)-score(a);
 });
}
export class PlayerEngine{
 ...
 next(recovering:boolean){
  ...
  const candidates=orderedSources(channel,this.local,this.attempted);const source=browserSources(channel).find(s=>s.id===this.preferredSourceId&&!this.attempted.has(s.id))||candidates[0];
