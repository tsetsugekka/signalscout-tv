...
export type Health={okAt?:number;verifiedLive?:boolean;suspect?:boolean;resolution?:number;failedAt?:number;failures:number;until:number};
...
export function recordFailure(state:LocalState,url:string,now=Date.now(),suspect=false) {const old=state.health[url];const failures=(old?.failures||0)+1;state.health[url]={...old,suspect,failedAt:now,failures,until:now+[10,30,120][Math.min(failures-1,2)]*60_000};}
export function recordSuccess(state:LocalState,url:string,now=Date.now()){state.health[url]={resolution:state.health[url]?.resolution,okAt:now,verifiedLive:true,failures:0,until:0};}
