...
“本机可播”表示当前浏览器有仍然有效的成功播放记录，不保证此刻或其他网络也能播放。后台线路探测不会覆盖你上次实际观看成功的线路偏好。
...
共享成功不按天数过期。成功后保护 24 小时；保护期内记录失败，满 24 小时且至少两个不同的其他浏览器报告后续失败，才取消共享可播。同一浏览器重复失败只计一次，新成功重新开始保护期。免登录环境以匿名浏览器标识近似区分使用者。本机失败一次就立即改为本机不稳定。“他人可播”汇总 PC 或手机已有可播记录的频道，不会启动全频道试播。目录更新确认地址移除时清除标记，重新收录后需要重新验证。
...
同域名或 IP 的其他线路可获得“可能可播”提示，忽略端口差异；这只是尚未验证的参考，不会覆盖明确失败，也不计入已验证可播。频道信号只反映当前设备适用的结果，后台检查备用线路不会让正常播放的频道闪动。
