...
无需应用账号。收藏、观看历史和线路验证记录保存在当前浏览器。项目不内置频道快照、不预置“可播”结果，也不录制、保存或转码视频。
...
5. 想减少列表中的失败项，可打开“不显示失效源”。未检测的线路仍会保留。
...
- 缓存有效期为 6 小时；可手动更新。单个目录更新失败时使用其上次有效内容，避免整个频道库消失。
...
- **服务端**：D1 保存频道目录元数据、更新状态及内部转发签名材料；签名密钥在运行时生成，不写入源码。
