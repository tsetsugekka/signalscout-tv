...
SignalScout TV 是一个面向浏览器的电视直播播放器。它在运行时读取 GitHub 上的 TXT / M3U 目录，将同一频道的不同线路合并并去重，合并时忽略画质标签、英文大小写与空格差异；选择频道后，先检查连接，再验证直播画面。你可以手动选择线路，也可以让播放器自动寻找可用连接。
...
| 手动选源 | 按固定线路编号切换，显示目录画质标签、连接耗时和检测结果；每页 20 条 |
