...
共享成功不按天数过期。成功后保护 24 小时；保护期内记录失败，满 24 小时且至少两个不同的其他浏览器报告后续失败，才取消共享可播。同一浏览器重复失败只计一次，新成功重新开始保护期。免登录环境以匿名浏览器标识近似区分使用者。本机失败一次就立即改为本机不稳定。“他人可播”汇总 PC 或手机已有可播记录的频道及“可能可播”频道，不会启动全频道试播。目录更新确认地址移除时清除标记，重新收录后需要重新验证。
