...
export async function GET(request:Request){
 try{
  ...
  if(!force&&cached)return json({...cached,origin:"cache",refreshNeeded:!cached.inputs?.includes(TVAPP_README)||now-cached.syncedAt>=SIX_HOURS});
  ...
  if(row?.synced_at){const latest=await readCachedPayload<Catalog>(row.payload);return json({...latest,stale:!!latest.stale||!!lease.meta.changes||now-row.synced_at>=SIX_HOURS,origin:lease.meta.changes?"stale":"cooldown",refreshNeeded:!latest.inputs?.includes(TVAPP_README)||now-row.synced_at>=SIX_HOURS});}
