...
import {catalogFromChannels,mergeChannels,CATALOG_CACHE_KEY,TVAPP_README,type Catalog} from "@/lib/catalog";
...
export async function GET(request:Request){
 try{
  ...
  const rawCached=row?.synced_at?await readCachedPayload<Catalog>(row.payload):null;
  const cached=rawCached?{...rawCached,channels:numberSources(mergeChannels([rawCached.channels]),rawCached.channels)}:null;
  ...
  if(row?.synced_at){const rawLatest=await readCachedPayload<Catalog>(row.payload);const latest={...rawLatest,channels:numberSources(mergeChannels([rawLatest.channels]),rawLatest.channels)};return json({...latest,stale:!!latest.stale||!!lease.meta.changes||now-row.synced_at>=ONE_HOUR,origin:lease.meta.changes?"stale":"cooldown",refreshNeeded:!latest.inputs?.includes(TVAPP_README)||now-row.synced_at>=ONE_HOUR});}
