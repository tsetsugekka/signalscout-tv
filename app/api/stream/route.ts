...
export async function GET(request:Request){
 try{
  ...
  const {response,url}=await fetchRedirects(target,request,checkPublicHost,fetch,kind==="asset");
  ...
  return new Response(response.body,{status:response.status,headers:assetHeaders(response.headers,url)});
